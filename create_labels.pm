...
#  my $affine_xform  = create_affine_transform ("${channel2}-reg2-whs-path", "${channel1}-reg2-whs-path", $Hf);
#  my $affine_xform  = create_affine_transform ($Hf);
  my $affine_xform  = create_two_channel_affine_transform ('T2W_reg2_${atlas_id}_path', 'T1_reg2_${atlas_id}_path', $Hf);

  my ($ds_affine_xform_base, $ds_warp_xform_base, $ds_inverse_warp_xform_base, $ants_transform_prefix) = create_diff_syn_transform ($affine_xform, $SYNSETTING , $Hf); #defines syn_setting as 0.5, we could save this to hf perhaps., or more fun, specify it elsewhere and store in headfile.
...
# this would change the funtion flow from if metric a, hardcode value b to load metrics, if metric a, value read b .
  ...
  my ($Hf) = @_;
  my $atlas_image_dir    = $Hf->get_value('dir-atlas-images');
  my @channel_array = split(',',$Hf->get_value('runno_ch_commalist'));
  my $channel1 = ${channel_array[0]};  
  my $channel2 = ${channel_array[1]};
#"${channel2}-reg2-whs-path", "${channel1}-reg2-whs-path", 
# --- find all the image files 
  my @list;
  $gcurrent_channel2_path = $Hf->get_value("${channel2}-reg2-${atlas_id}-path");
  $gcurrent_channel1_path = $Hf->get_value("${channel1}-reg2-${atlas_id}-path");  
  push @list, $gcurrent_channel2_path;
  push @list, $gcurrent_chanenl1_path;
#  push @list, $Hf->get_value("${channel2}-reg2-${atlas_id}-path");
#  push @list, $Hf->get_value("${channel1}-reg2-${atlas_id}-path");
  $gwhs_channel2_path = "$atlas_image_dir/${atlas_id}_${channel2}_ln.nii";
  $gwhs_channel1_path = "$atlas_image_dir/${atlas_id}_${channel1}_ln.nii";
  push @list, $gwhs_channel2_path;
  push @list, $gwhs_channel1_path;
...
# ------------------
sub create_two_chanel_affine_transform {
# ------------------
